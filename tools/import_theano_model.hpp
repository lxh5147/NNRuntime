...
namespace nn {
    namespace tools {
        ...
        class NPYData {
            ...
            private:
                void loadData(){
                    ifstream is(m_file.c_str(),ios::binary);
                ...
                void loadHeaderAndShape(ifstream& is,int& row, int& col){
                    ...
                    *end_shape = 0;
                    col=1;
                    int dim = sscanf(shape, " %d , %d ", &row, &col);
        ...
        class TheanoModel{
            public:
                static void load(const string& path, vector<shared_ptr<InputInfo<T> > >& inputsInfo, vector<shared_ptr<Matrix<T> > >& weights, vector<shared_ptr<Vector<T> > >& biasVectors, vector<size_t>& activationFunctionIds, bool includeOutputSoftmaxLayer=false){
                    ...
                    BEGIN_STD_VECTOR_FOR(theanoModel.m_sequenceFeatures,feature,string)
                        theanoModel.loadInputInfo(feature,inputsInfo,true);
                    END_STD_VECTOR_FOR
                    BEGIN_STD_VECTOR_FOR(theanoModel.m_nonSequenceFeatures,feature,string)
                        theanoModel.loadInputInfo(feature,inputsInfo,false);
                    END_STD_VECTOR_FOR
                    //prepare layers
                    size_t activationFunctionId=getActivationFunctionId(theanoModel.m_activationFunction);
            ...
            private:
                 void loadInputInfo(const string& feature, vector<shared_ptr<InputInfo<T> > >& inputsInfo, bool isSequenceFeature){
                    ...
                    shared_ptr<Matrix<T> > embedding=loadMatrix(getFilePath(m_path,embeddingFile));
                ...
                void loadLayer(const string& name, vector<shared_ptr<Matrix<T> > >& weights,vector<shared_ptr<Vector<T> > >& biasVectors){
                    ...
                    weightFile+=name;
                    weightFile+=".npy";
                    shared_ptr<Matrix<T> > weight=loadMatrix(getFilePath(m_path,weightFile));
                    ...
                    vectorFile+=name;
                    vectorFile+=".npy";
                    shared_ptr<Vector<T> > biasVector=loadVector(getFilePath(m_path,vectorFile));
                ...
                static shared_ptr<Vector<T> > loadVector(const string& npyFile){
                    shared_ptr<Matrix<T> > pMatrix=NPYData<T>(npyFile).getData();
        ...
        void import_theano_model(const string& inputTheanoModelFolder, const string& outputRuntimeModelFile, bool includeOutputSoftmaxLayer){
            ...
            MLPModelFactory::save<T>(outputRuntimeModelFile,inputsInfo,weights,biasVectors,activationFunctionIds);
        }
